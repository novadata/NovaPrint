 ...
 * */	
		...
		{
			var gridXml:XML=LayoutMap.getSimple().gridXml;
			var dataGridXml:XML=<DataGrid></DataGrid>;
			dataGridXml.@x=gridXml.@x;
			dataGridXml.@y=gridXml.@y;
			dataGridXml.@id=gridXml.@id;
			dataGridXml.@width=gridXml.@width;
			dataGridXml.@height=gridXml.@height;
			var columnXml:XML=new XML(<columns></columns>);
			for(var i:int=0;i<gridXml.children().length();i++)
			{
				var xml:XML=gridXml.children()[i] as XML;
				var field:String=xml.@dataField;
				var gridWidth:int=getRecordGridWidth(field);
				if(gridWidth!=0)
				{
					xml.@width=gridWidth;
				}
				columnXml.appendChild(<DataGridColumn  visible={xml.@visible} textAlign={xml.@textAlign} width={xml.@width} dataField={xml.@dataField} headerText={xml.@headerText}/>);
			}
			dataGridXml.appendChild(columnXml);
			trace("--------------------------------dataGridXml:  "+dataGridXml+"|");
			return dataGridXml;
		}
			...
			for(var i:int=0;i<SetupInfo.getInstance().colPages;i++)
			{
				for(var j:int=0;j<SetupInfo.getInstance().rowPages;j++)
				{
					if(j%2==1)
					{
						...
						doc.Creat(j,i,SetupInfo.getInstance().gridHeight);
 ...
 * */
		...
		private function printCurrentPage():Array
		{
			var array:Array=[];
			var doc:PrintDoc=new PrintDoc();
			doc.Creat(SetupInfo.getInstance().rowCurrentPage-1,SetupInfo.getInstance().colCurrentPage-1,SetupInfo.getInstance().gridHeight);
			trace("打印当前页面----------------------------------------start");
		...
		private function printPart():Array
		{
			...
			if(array.length==1)
			{
				...
				doc.Creat(array[0]-1,colCurrentPage-1,SetupInfo.getInstance().gridHeight);
		...
		private function printPartOther():Array
		{
			...
			for(var i:int=f;i<=e;i++)
			{
				...
				var doc:PrintDoc=new PrintDoc();
				doc.Creat(i-1,colCurrentPage-1,SetupInfo.getInstance().gridHeight);
				trace("打印的页码为: "+i);
				trace("打印的数据的当前页为： "+colCurrentPage);
				docArry.push(doc);
			}
		...
		private function colToRow():Array
		{
			...
			for(var i:int=0;i<SetupInfo.getInstance().rowPages;i++)
			{
				for(var j:int=0;j<SetupInfo.getInstance().colPages;j++)
				{
					...
					doc.Creat(i,j,SetupInfo.getInstance().gridHeight);
					array.push(doc);                                 
				}
			}
			trace("打印所有页面---------------------------------------------------end");
