package com.nova.print.doc
{
	...
	public class PrintText extends Sprite
	{
		...
		private function creat():void
		{
			...
			for(var i:int=0;i<xml.children().length();i++)
			{
				...
				if(ID.length>0)
				{
					if(wrap.length>0)
					{
						creatWrapTxt(ID,tx,ty,size,width);
					}
					else
					{
						var nt:NovaText=new NovaText(findById(ID),"left",size,width);
						nt.x=tx;
						nt.y=ty;
						//trace("加载动态赋值的数据为:  "+findById(ID)+"   X: "+tx+"    Y:"+ty);
						this.addChild(nt);
					}
				}
		...
		private function creatWrapTxt(ID:String,tx:int,ty:int,size:String,width:int):void
		{
